...
let originalImage = availableImages[0].src; // 초기 이미지 설정
let tiles = [];
let timerInterval;
let time = 0;
let moves = 0;
...
window.onload = function() {
  setGridSize(3);
  createImageSelector(); // 이미지 선택 UI 생성
};
...
function dragStart(e) {
  ...
  e.dataTransfer.setData('text/plain', `<span class="math-inline">\{e\.target\.dataset\.x\},</span>{e.target.dataset.y}`);
...
function drop(e) {
  ...
  // 타일 찾기
  const draggedTile = document.querySelector(`.tile[data-x="<span class="math-inline">\{tileX\}"\]\[data\-y\="</span>{tileY}"]`);
