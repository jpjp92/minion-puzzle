...
function checkComplete() {
  ...
  if (placedTiles.length === gridSize * gridSize) {
    ...
    const difficulty = gridSize === 3 ? 'Easy' : gridSize === 4 ? 'Medium' : 'Hard';
    const score = calculateScore(difficulty, time, moves);

    saveScore(nickname, difficulty, time, moves, score);
    showLeaderboardModal(score, difficulty, time, moves);
    showMessage(`축하합니다! ${time}초, ${moves}회`, 'success', 5000);

    placedTiles.forEach((tile, idx) => setTimeout(() => tile.classList.add('complete'), idx * 100));
  }
}

function calculateScore(difficulty, timeTaken, moves) {
  ...
  const rawScore = baseScore - timePenalty - movePenalty;
  const weightedScore = rawScore * (difficultyWeights[difficulty] || 1);
  return Math.max(Math.floor(weightedScore), 0);
}

async function saveScore(nickname, difficulty, time, moves, score) {
  try {
...
function showLeaderboardModal(score, difficulty, timeTaken, moves) {
  const nickname = localStorage.getItem('nickname') || 'Unknown';
  ...
  modal.innerHTML = `
    <div class="leaderboard-modal">
      <h2>🎉 Game Over</h2>
      <p><strong>${nickname}</strong>님의 점수: <strong>${score}</strong></p>
      <p>난이도: ${difficulty} / 시간: ${timeTaken}s / 이동: ${moves}회</p>
      <button onclick="this.closest('.leaderboard-modal-overlay').remove()">닫기</button>
    </div>
  `;
